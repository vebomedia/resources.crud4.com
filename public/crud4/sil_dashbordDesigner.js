...
//CONTAINER_TAB.prototype.addColumn = function (column_object) {
//
//    this.columns.push(column_object);
//    return this;
//};
...
CONTAINER_ROW.prototype.serialize = function ($element) {
    ...
    $element.children('.CONTAINER_COL').each(function (index2, value2) {
        ...
        obj['col_' + index2] = s_CONTAINER_COL($(this));
...
function s_MAIN_CONTAINER($element) {
    ...
    $element.children('.CONTAINER_ROW').each(function (index, value) {
        ...
        var $d = new CONTAINER_ROW();
        out['ROWS']['row_' + index] = $d.serialize($CONTAINER_ROW);


    });
    return out;
}
function s_CONTAINER_ROW($element) {
    ...
    $element.children('.CONTAINER_COL').each(function (index2, value2) {
        ...
        var $CONTAINER_COL = $(this);

        if (typeof out['COLS'] === 'undefined') {
            out['COLS'] = {};
        }

        out['COLS']['col_' + index2] = s_CONTAINER_COL($CONTAINER_COL);
...
function s_CONTAINER_COL($element) {
    ...
    $element.children('.ELEMENT').each(function (index3, value3) {
        ...
        if (typeof out['ELEMENTS'] === 'undefined') {
            out['ELEMENTS'] = {};
        }

        out['ELEMENTS']['element_' + index3] = s_ELEMENT($ELEMENT);
...
function s_ELEMENT($element) {
    ...
    $element.find('.ELEMENT_CONTENT').find('.tab-content').children('.MAIN_CONTAINER').each(function (index4, value4) {
        ...
        if (typeof out['TABS'] === 'undefined') {
            out['TABS'] = {};
        }

        out['TABS']['tab_' + index4] = s_MAIN_CONTAINER(MAIN_CONTAINER);
    });
